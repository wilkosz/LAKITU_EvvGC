...
//debug settings
int debugPrint  = 0;//changed for digital implementation, forces print all values
int debugPerf   = 0;
int debugSense  = 0;
int debugCnt    = 0;
int debugRC     = 0;
...
void engineProcess(float dt)
{
    ...
    rollRCOffset += Step[ROLL] / 1000.0;// include the config roll offset which is scaled to 0 = -10.0 degrees, 100 = 0.0 degrees, and 200 = 10.0 degrees
    ...
    if (configData[10] == '0' && !LAKITU_enable)
    {
    ...
    if (printcounter >= 500)
    {
        ...
        if (debugLAKITU){
             print("LAKITU_pitch:%3.2f | LAKITU_roll:%3.2f | LAKITU_yaw:%3.2f\n\r",
            	   LakituAngles[PITCH]-LAKITU_CT_VALUE,
                   LakituAngles[ROLL],
                   LakituAngles[YAW]-LAKITU_CT_VALUE_YAW);
        }
